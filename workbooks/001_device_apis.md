...
JSON instead of screen-scraped text. The workbook closes with a small client
class that wraps the call, the same client every later workbook reuses.
...
JSON-RPC result instead. Arista's eAPI listens for an HTTPS `POST` at
`/command-api`; the body names the CLI commands to run and the format to return
them in. This exercise calls your own lab leaf.
